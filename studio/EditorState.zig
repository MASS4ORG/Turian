...
pub var debug_metrics: introspect.Metrics = .{};
...
pub fn refreshDebugMetrics() void {
    debug_metrics = introspect.Metrics.fromProfiler(engine.Profiler.captured());
    var comps: u32 = 0;
    for (0..object_count) |i| comps += @intCast(objects[i].component_count);
    debug_metrics.withScene(if (scene_open) 1 else 0, @intCast(object_count), comps);
}
...
pub fn refreshDebugAssets() []const introspect.AssetView {
    if (!asset_db_initialized) return &.{};
    var n: usize = 0;
    var it = asset_db.by_guid.valueIterator();
    while (it.next()) |info| {
...
pub fn debugReloadAsset(io: std.Io, arena: std.mem.Allocator, guid_str: []const u8) bool {
    if (!asset_db_initialized) return false;
    const guid = editor.Guid.parse(guid_str) catch return false;
    if (!asset_db.by_guid.contains(guid)) return false;
...
pub fn debugSetComponentField(now: i128, idx: usize, component: []const u8, field: []const u8, value: introspect.Value) bool {
    ...
    const ci = introspect.componentIndex(&objects[idx], component) orelse return false;
    const before = objects[idx];
    if (!introspect.setComponentField(&objects[idx].components[ci], field, value)) return false;
    const after = objects[idx];
    pushCommand(now, &.{ .modify_object = .{ .idx = idx, .before = before, .after = after } });
    scene_dirty = true;
    return true;
}
...
pub fn debugSetTransform(now: i128, idx: usize, channel: []const u8, value: [3]f32) bool {
    if (idx >= object_count) return false;
    const before = objects[idx];
    if (!introspect.setTransformField(&objects[idx], channel, value)) return false;
    const after = objects[idx];
    pushCommand(now, &.{ .modify_object = .{ .idx = idx, .before = before, .after = after } });
    scene_dirty = true;
    return true;
