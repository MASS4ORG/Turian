...
const AssetInfo = @import("../assets/AssetDatabase.zig").AssetInfo;
const asset_importer = @import("../assets/AssetImporter.zig");
const asset_packager = @import("../assets/AssetPackager.zig");
...
const ProjectDependency = @import("../project/ProjectConfig.zig").Dependency;
    ...
    if (it.next()) |info| {
        if (readFileAlloc(io, a, info.path)) |bytes| {
            if (engine.ProjectSettings.loadFromBytes(a, bytes)) |ps| {
                ...
                out.frame_cap = ps.graphics.frame_cap;
