//! Runtime introspection layer (issue #2) — the single source of truth for
...
//! Remote Debug Protocol (#49) builds the `World` from the live `SceneManager`
//! and forwards these calls; the MCP server (#50) sits on top of that. Keeping
...
/// with full detail. This is the serialisable "everything" form (issue #2:
...
/// field types, and enum value sets. Agents (#50) and tools use this to
